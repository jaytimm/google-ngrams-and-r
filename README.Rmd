...
library(tidyverse)
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/matrices")
tfms_mats <- readRDS('tfms_mats.rds')

setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R")
freqs_by_gen <- readRDS('freqs_by_gen.rds')

search <- toupper('grasp')
x <- lapply(tfms_mats, LSAfun::neighbors, x = search , n = 100)
...
unzipped_eg <- data.table::fread("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/data/egs/googlebooks-eng-1M-5gram-20090715-1.csv") 
set.seed(1999)
unzipped_eg %>%
  sample_n(5) %>%
  knitr::kable()
...
```{r echo=FALSE}
data.table::fread("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/data/egs/eng-1M-5gram-20090715-1.csv") %>%
  sample_n(5) %>%
  knitr::kable()
```
...
```{r include=FALSE}
local_raw <- "/home/jtimm/Desktop/raw/"
```
...
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\data")
#saveRDS(grams, 'grams.rds')
```


```{r eval=FALSE, include=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/data")
grams <- readRDS('grams.rds')

#Output example.
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\data")
#saveRDS(grams[[2]], 'grams_eg.rds')
```
...
```{r echo=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/data/egs")
grams_eg <- readRDS('grams_eg.rds')
head(grams_eg) %>% knitr::kable()
```
...
```{r echo=FALSE}
summary %>% mutate(prop_tokens = round(tokens/sum(tokens),3),
                   prop_ngrams = round(ngrams/sum(ngrams),3)) %>%
  knitr::kable()
```
...
```{r eval=FALSE, include=FALSE}
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\data")
#saveRDS(grams, 'grams_by_gen.rds')
```
...
```{r eval=FALSE, include=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/data")
grams <- readRDS('grams_by_gen.rds')
```
...
#saveRDS(tfms, 'tfms.rds')
```


```{r include=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/matrices")
tfms <- readRDS('tfms.rds')
```
...
lapply(tfms, dim) %>%
  data.table::melt(.) %>%
  bind_cols(type = rep(c('terms','features'), 8)) %>%
  spread(type, value) %>%
  rename(quarter = L1) %>%
  select(quarter, terms, features)%>%
  knitr::kable()
```
...
```{r include=FALSE}
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\data")
#saveRDS(freqs_by_gen, 'freqs_by_gen.rds')
```
...
```{r include=FALSE}
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\matrices")
#saveRDS(tfms_filtered, 'tfms_filtered.rds')
```


```{r include=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/matrices")
tfms_filtered <- readRDS('tfms_filtered.rds')
```
...
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/matrices")
saveRDS(tfms_ppmi, 'tfms_ppmi.rds')
```
...
```{r include=FALSE}
setwd("/home/jtimm/jt_work/GitHub/git_projects/google_ngrams_and_R/matrices")
tfms_ppmi <- readRDS('tfms_ppmi.rds')
```


Here, we build a network structure for a set of forms with some overlapping semantic structure.  Note: It probably makes more sense to use the `igraph` package for extracting subgraphs from larger networks, 1/19/20.
...
```{r eval=FALSE, include=FALSE}
#names(tfms_svd) <- names(tfms_ppmi)
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\matrices")
#saveRDS(tfms_svd, 'tfms_svd.rds')
```
...
```{r eval=FALSE, include=FALSE}
#names(tfms_mats) <- names(tfms_ppmi)
#setwd("C:\\Users\\jason\\Google Drive\\GitHub\\git_projects\\google_ngrams_and_R\\matrices")
